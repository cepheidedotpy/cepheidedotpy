...
try:
    ...
    print('Status Error \n')
# finally:
#     rf_Generator.visa_timeout = 5000
...
def handle_event(ressource, event, user_handle):
    ressource.called = True
    print(f"Handled event {event.event_type} on {ressource} \n")
    pass


def bias_voltage(
        voltage):  # Set bias voltage from user input to correspond to signal generator input. [sig_gen_voltage =
...
def connect():
    ...
    sig_gen = rm.open_resource('TCPIP0::A-33521B-00526::inst0::INSTR')
    osc = rm.open_resource('TCPIP0::DPO5054-C011738::inst0::INSTR')
    # rf_Generator = RsInstrument('TCPIP0::rssmb100a179766::inst0::INSTR')
...
def cycling_sequence(number_of_cycles=200, number_of_pulses_in_wf=20, filename='test', wf_duration=0.02):
    ...
    try:
        with open("{}.csv".format(filename), mode='x') as csvfile:
            ...
            writer.writeheader()
            writer.writerow({'cycles': cycles, 'Vpull_in(V)': cycling_data[0], 'Vpullout (V)': cycling_data[1],
                             'Isolation (dB)': cycling_data[2], 'sw_time (s)': cycling_data[3]})
    except:
